...
const statuses = ["RECEIVED", "ACCEPTED", "PREPARING", "READY", "READY_FOR_PICKUP", "SERVED", "OUT_FOR_DELIVERY", "COMPLETED", "CANCELLED"];
const orderFilters = ["ALL", "DINE_IN", "COLLECTION", "DELIVERY"];
const paymentFilters = ["ALL", "PAID", "PAY_IN_STORE", "PENDING", "FAILED", "REFUNDED"];
  ...
  return (
    <div className="mt-8 space-y-5">
      ...
      <div className="overflow-hidden rounded-lg border border-date/10 bg-white shadow-sm">
        ...
        {orders.length ? (
          orders.map((order) => (
            <div key={order.id} className="grid gap-4 border-t border-date/10 px-5 py-4 md:grid-cols-[1fr_150px_140px_180px] md:items-center">
              <div>
