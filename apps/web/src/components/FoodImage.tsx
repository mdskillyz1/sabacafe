export function FoodImage({ label, src, className = "" }: { label: string; src?: string; className?: string }) {
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-date/70 to-transparent p-4 text-cream sm:p-5">
        <p className="break-words font-display text-lg font-semibold leading-tight sm:text-xl">{label}</p>
      </div>
    </div>
  );
}
